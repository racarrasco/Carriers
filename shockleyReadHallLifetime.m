function[tauSRH] = shockleyReadHallLifetime(temp, type,meStar,mhStar,Ev,Ec,Et,sigmaN,dopingDensity)
...
%make the temperature a row since the band gaps are a row
if(size(temp,1) > 1)
    x = temp';
else
    x = temp;
